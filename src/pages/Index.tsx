...
import FeatureLongWeekends from "@/components/feature-long-weekends";
    ...
    <div className="min-h-screen flex flex-col bg-background overflow-x-hidden">
      ...
      <main className="flex-grow overflow-y-auto -webkit-overflow-scrolling-touch">
          ...
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:gap-4 text-center">
              <div className="p-2 sm:p-4">
                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-trypie-600 mb-1">10K+</p>
                <p className="text-xs sm:text-sm text-gray-600 flex items-center justify-center"><Globe className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> Destinations</p>
              </div>
              <div className="p-2 sm:p-4">
                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-trypie-600 mb-1">50K+</p>
                <p className="text-xs sm:text-sm text-gray-600 flex items-center justify-center"><Users className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> Happy Travelers</p>
              </div>
              <div className="p-2 sm:p-4">
                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-trypie-600 mb-1">250+</p>
                <p className="text-xs sm:text-sm text-gray-600 flex items-center justify-center"><Calendar className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> Trips Daily</p>
              </div>
              <div className="p-2 sm:p-4">
                <p className="text-xl sm:text-2xl md:text-3xl font-bold text-trypie-600 mb-1">1.2M+</p>
                <p className="text-xs sm:text-sm text-gray-600 flex items-center justify-center"><MapPin className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> Local Experiences</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="py-8 md:py-12 bg-white">
          <div className="container mx-auto px-4">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 md:mb-8 gap-2 sm:gap-4">
              <div>
                <span className="text-trypie-600 font-medium mb-1 sm:mb-2 block text-sm sm:text-base">PLAN AHEAD</span>
                <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">
                  2025 Holiday Calendar
                </h2>
              </div>
              <Button variant="outline" asChild className="border-trypie-500 text-trypie-600 text-sm">
                <Link to="/long-weekends">View Full Calendar</Link>
              </Button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="md:col-span-2 bg-gray-50 rounded-xl md:rounded-2xl p-3 sm:p-4 md:p-6 shadow-md">
                  ...
                  <div className="space-y-2 sm:space-y-3 md:space-y-4">
                    {[
                      { 
                        ...
                        days: 3, 
                        destinations: ["Mathura", "Vrindavan", "Jaipur"],
                        badge: "No Leave Required"
                      },
                      {
                        holiday: "Mahavir Jayanti", 
                        dates: "April 10-13, 2025", 
                        days: 4,
                        destinations: ["Palitana", "Mount Abu", "Ranakpur"],
                        badge: "Take Leave: Apr 11"
                      },
                      {
                        holiday: "Good Friday", 
                        dates: "April 18-20, 2025", 
                        days: 3,
                        destinations: ["Goa", "Pondicherry", "Kochi"],
                        badge: "No Leave Required"
                      }
                    ].map((weekend, index) => (
                      <div key={index} className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-white rounded-lg border">
                        <div>
                          <div className="flex flex-wrap gap-2 items-center mb-2">
                            <h4 className="font-medium">{weekend.holiday}</h4>
                            <span className="text-sm text-gray-500">({weekend.dates})</span>
                            <Badge className={weekend.badge.includes("No") ? "bg-green-500" : "bg-yellow-500 text-gray-800"}>
                              {weekend.badge}
                            </Badge>
                          </div>
                          <p className="text-sm text-gray-600">
                            <span className="font-medium">Top destinations:</span> {weekend.destinations.join(", ")}
                          </p>
                        </div>
                        <Button variant="outline" size="sm" asChild className="mt-3 md:mt-0">
                ...
                <div className="mt-3 md:mt-4 md:mt-6 text-center">
                  <Button asChild size="sm" className="text-xs sm:text-sm">
                    <Link to="/long-weekends">
                      <Calendar className="mr-1 sm:mr-2 h-3 w-3 sm:h-4 sm:w-4" />
                      View All 2025 Holidays
                    </Link>
                  </Button>
                </div>
              </div>
        ...
        <section className="py-10 md:py-16 bg-gray-50">
          <div className="container mx-auto px-2 sm:px-4 md:px-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-7 md:mb-10 gap-4">
              <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
              {[
                ...
                <div key={index} className="relative group overflow-hidden rounded-xl md:rounded-2xl bg-white shadow hover:shadow-lg transition-all duration-200">
                  <div className="aspect-[4/5] w-full">
                    <img 
                      src={destination.image} 
                      alt={destination.name}
                      className="w-full h-full object-cover transition-transform group-hover:scale-110 duration-200 rounded-xl md:rounded-2xl"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent"></div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4 text-white">
                    <h3 className="font-bold text-base md:text-lg">{destination.name}</h3>
                    <p className="text-xs md:text-sm flex items-center"><MapPin className="mr-1 h-3 w-3" /> {destination.country}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* App Promotion */}
        <section className="py-10 md:py-16 bg-gradient-to-r from-trypie-600 to-trypie-700 text-white">
          <div className="container mx-auto px-2 sm:px-4 md:px-6">
            <div className="flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="md:w-5/12">
                <img 
                  ...
                  alt="Mobile App" 
                  className="w-full max-w-xs mx-auto rounded-xl md:rounded-3xl shadow-2xl"
                />
              </div>
            </div>
          </div>
