...
const ChatInput = ({ onSendMessage, isLoading, groupId }: ChatInputProps) => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // First check if bucket exists, if not, instruct user properly
        ...
        toast({
          ...
          description: (
            <div className="flex items-center gap-2">
              <img src="/lovable-uploads/3b42be25-3f03-4111-9442-8a7a7c450f60.png" alt="Setup" className="h-8 w-8 rounded" />
              <span>Please ask an administrator to set up the chat media storage for group files.</span>
            </div>
          ),
        });
      ...
      <div className="flex items-end gap-2">
        <div className="flex gap-1">
          ...
          <Button 
            variant="ghost" 
            size="icon" 
            className={`shrink-0 ${isUploading ? "opacity-50" : ""}`}
            title="Add image"
        ...
        <Textarea
          ...
          className="min-h-[40px] max-h-[120px] flex-1 bg-gray-50"
          rows={1}
        ...
        <Button 
          ...
          className={cn(
            "shrink-0 bg-trypie-600 hover:bg-trypie-700",
